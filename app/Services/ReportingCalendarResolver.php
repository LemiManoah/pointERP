...
final class ReportingCalendarResolver
{
    ...
    {
        $calendar = $this->calendarFor($site);
        $timezone = $calendar?->timezone ?? $this->tenantContext->current()->timezone;
        $deadline = $calendar?->reporting_deadline
            ?? $site->reporting_deadline
            ?? $site->project?->reporting_deadline
            ?? '18:00:00';
    ...
    {
        return max(1, $this->calendarFor($site)?->missing_escalation_days ?? 2);
    }
}
