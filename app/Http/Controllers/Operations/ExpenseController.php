...
final class ExpenseController
{
    ...
    public function show(Request $request, Expense $expense, ExpenseFormOptions $options): Response
    {
        ...
        return Inertia::render('operations/expenses/show', [
            ...
            'dsrCostLines' => DailySiteReportCostLine::query()->with('report')->whereHas('report', fn (Builder $query): Builder => $query->whereIn('project_id', $projectIds))->whereDoesntHave('expenseReconciliation')->latest()->get()->map(fn (DailySiteReportCostLine $line): array => ['value' => $line->id, 'label' => ($line->report?->reference ?? 'DSR cost').' - '.$line->description, 'amount' => $canViewCosts ? $line->amount : null]),
            ...$options->for($user),
