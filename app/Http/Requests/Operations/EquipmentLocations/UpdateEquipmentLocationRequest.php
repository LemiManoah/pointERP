...
final class UpdateEquipmentLocationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $location = $this->route('equipment_location');
        $id = $location instanceof EquipmentLocation ? $location->id : null;
        ...
        return [
            ...
            'site_id' => ['nullable', 'uuid', Rule::exists((new Site)->getTable(), 'id')->where('tenant_id', $tenantId), Rule::unique((new EquipmentLocation)->getTable(), 'site_id')->where('tenant_id', $tenantId)->ignore($id)],
            'type' => ['required', 'string', Rule::in(EquipmentLocation::TYPES)],
            'code' => ['required', 'string', 'max:40', Rule::unique((new EquipmentLocation)->getTable(), 'code')->where('tenant_id', $tenantId)->ignore($id)],
