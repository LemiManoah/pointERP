...
export function ReconcileDialog({
    ...
    const [open, setOpen] = useState(false);
    const form = useForm({ daily_site_report_cost_line_id: '', reason: '' });
    function submit(event: FormEvent) {
        event.preventDefault();
        form.post(`/expense-lines/${expenseLineId}/reconcile-dsr`, {
            preserveScroll: true,
            onSuccess: () => setOpen(false),
        });
    }
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-xl">
                <DialogHeader>
                    <DialogTitle>Match provisional DSR cost</DialogTitle>
                    <DialogDescription>
                        This prevents the DSR cost and approved expense from
                        being counted twice.
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={submit} className="grid gap-4">
                    <Field
                        label="DSR cost"
                        required
                        error={form.errors.daily_site_report_cost_line_id}
                    >
                        <SearchableSelect
                            value={form.data.daily_site_report_cost_line_id}
                            onValueChange={(value) =>
                                form.setData(
                            ...
                            options={dsrCostLines}
                            searchPlaceholder="Search DSR costs..."
                        />
                    </Field>
                    <Field label="Reason" required error={form.errors.reason}>
                        <Textarea
                    ...
                    <div className="flex justify-end">
                        <Button type="submit">Match cost</Button>
                    </div>
                </form>
            </DialogContent>
        </Dialog>
