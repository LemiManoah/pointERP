        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-w-xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Group materials for search, reporting and stock
                        controls.
                    </DialogDescription>
                </DialogHeader>
    ...
    const { form, submit } = useModalForm(
        {
            ...
            name: unit?.name ?? '',
            symbol: unit?.symbol ?? '',
            quantity_dimension: unit?.quantity_dimension ?? 'count',
            is_base_unit: unit?.is_base_unit ?? false,
            is_active: unit?.is_active ?? true,
        },
        unit ? `/inventory/units/${unit.id}` : '/inventory/units',
        unit ? 'put' : 'post',
        () => setOpen(false),
    );
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button
                    variant={unit ? 'outline' : 'default'}
                    size={unit ? 'sm' : 'default'}
                >
                    {unit ? <Pencil /> : <Plus />}
                    {unit ? 'Edit' : 'New unit'}
                </Button>
            ...
            <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-w-xl">
                <DialogHeader>
                    <DialogTitle>
                        {unit ? 'Edit unit of measure' : 'New unit of measure'}
                    </DialogTitle>
                    <DialogDescription>
                        Define a measurement that inventory items can use as
                        their stock unit.
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={submit} className="grid gap-5">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Field label="Symbol" error={form.errors.symbol}>
                            <Input
                        ...
                        <Field
                            label="Dimension"
                            error={form.errors.quantity_dimension}
                        >
                            <NativeSelect
                                value={form.data.quantity_dimension}
                                onChange={(e) =>
                                    form.setData(
                                        'quantity_dimension',
                                        e.target
                            ...
                            >
                                <NativeSelectOption value="mass">
                                    Mass
                                </NativeSelectOption>
                                <NativeSelectOption value="volume">
                                    Volume
                                </NativeSelectOption>
                                <NativeSelectOption value="length">
                                    Length
                                </NativeSelectOption>
                    ...
                    <div className="flex flex-wrap gap-5 text-sm">
                        <label className="flex items-center gap-2">
                            <input
                                type="checkbox"
                                checked={form.data.is_base_unit}
                                onChange={(e) =>
                                    form.setData(
                                        'is_base_unit',
                                        e.target.checked,
                ...
                <form onSubmit={submit} className="grid gap-5">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        >
                            ...
                            >
                                <NativeSelectOption value="construction_material">
                                    Construction material
                                </NativeSelectOption>
                                <NativeSelectOption value="consumable">
                                    Consumable
                                </NativeSelectOption>
                                <NativeSelectOption value="spare_part">
                                    Spare part
                                </NativeSelectOption>
                                <NativeSelectOption value="fuel_related">
                                    Fuel related
                                </NativeSelectOption>
                        ...
                        >
                            ...
                            >
                                ...
                                <NativeSelectOption value="serial">
                                    Serial
                                </NativeSelectOption>
                                <NativeSelectOption value="batch">
                                    Batch
                                </NativeSelectOption>
                                <NativeSelectOption value="other">
                                    Other
...
export function StoreDialog({
    ...
    const { form, submit } = useModalForm(
        {
            ...
            equipment_location_id: store?.equipment_location_id ?? '',
            project_id: store?.project_id ?? '',
            site_id: store?.site_id ?? '',
            code: store?.code ?? '',
            name: store?.name ?? '',
            type: store?.type ?? 'depot',
            address: store?.address ?? '',
            is_active: store?.is_active ?? true,
        },
        store ? `/inventory/stores/${store.id}` : '/inventory/stores',
        store ? 'put' : 'post',
        () => setOpen(false),
    );
    ...
    const siteOptions = sites
        .filter(
            (row) =>
                (!form.data.branch_id ||
                    row.branch_id === form.data.branch_id) &&
                (!form.data.project_id ||
                    row.project_id === form.data.project_id),
        )
        .map((row) => ({
            value: row.id,
            label: row.name,
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button
                    ...
                    size={store ? 'sm' : 'default'}
                >
                    {store ? <Pencil /> : <Plus />}
                    {store ? 'Edit' : 'New store'}
                </Button>
            </DialogTrigger>
            <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-w-3xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        A store can serve several sites. The project and site
                        fields describe its primary context only.
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={submit} className="grid gap-5">
                    <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <Field label="Store type" error={form.errors.type}>
                            <NativeSelect
                            ...
                            >
                                ...
                                <NativeSelectOption value="depot">
                                    Depot
                                </NativeSelectOption>
                                <NativeSelectOption value="site_store">
                                    Site store
                                </NativeSelectOption>
                                <NativeSelectOption value="temporary">
                                    Temporary
                                </NativeSelectOption>
                                <NativeSelectOption value="other">
                                    Other
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <Field label="Site context" error={form.errors.site_id}>
                            <SearchableSelect
                                value={form.data.site_id}
                                options={[
                                    { value: '', label: 'No site context' },
                                    ...siteOptions,
                                ]}
                                onValueChange={(value) =>
