...
Phase 3A.4, Maintenance Control, is implemented and awaiting migration, UI and automated-test acceptance. It includes separate schedule, work-order and part-snapshot tables; date/meter/whichever-first due calculations; planned/approved/in-progress/completed/cancelled controls; independent approval; equipment availability locking and explicit release; completion meter posting; provider, downtime, parts and cost evidence; schedule recalculation; a scoped maintenance portfolio with filters and summaries; audited CSV export; idempotent scheduled reminders; role-aware cost omission; controlled work-order evidence; audit, notifications, an equipment Maintenance tab and seeded current/overdue/completed/pending/compliance examples. Phase 3A.5 remains for final DSR integration and management-reporting hardening.

Phase 3A.5a, Project/Site Fleet Visibility and Reconciliation, is implemented and awaiting UI and automated-test acceptance. Project and site pages now separate current accepted deployment from recent ledger activity and approved DSR history; show 30-day working hours and net fuel, open maintenance, unavailable assets and DSR posting exceptions; drill into equipment and reports; and omit the entire fleet payload without equipment permission. Phase 3A.5b remains for additive corrections to fleet facts originating from approved DSR lines and the final Phase 3A isolation/acceptance pass.

Phase 3A.5b, Additive DSR Fleet Correction, is implemented and awaiting migration, UI and automated-test acceptance. Approved DSR snapshots and their original posted usage/fuel entries remain immutable. The existing two-person DSR correction workflow can now propose signed working-hour, idle-hour and fuel deltas for posted linked equipment lines; approval appends usage and fuel entries, records an immutable correction-to-line ledger, prevents negative resulting totals and duplicate posting, audits the reason and generated records, and exposes corrected net values in project/site reconciliation. Meter changes continue through the dedicated meter-correction workflow. The planned Phase 3A implementation scope is now complete pending the local quality suite and acceptance review.
